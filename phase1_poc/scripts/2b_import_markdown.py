#!/usr/bin/env python3
"""
Phase 1 POC - Script 2b: Import Textbooks from Markdown with Entity Extraction
===============================================================================

This script imports textbook content from Markdown files into CozoDB with
TypeAgent structured RAG. Markdown is cleaner than PDF extraction.

Process:
1. Reads markdown files (pre-converted from PDFs)
...
Place markdown files in ./data/textbooks_md/ with naming convention:
    {subject}_{grade}_chapter{number}.md

Examples:
    - matematika_7_chapter1.md
    - fisika_8_chapter2.md
    - kimia_9_chapter3.md

Advantages over PDF import:
- Cleaner text extraction (no PDF parsing issues)
- Preserves markdown formatting
- Easier to edit and curate content
- Faster processing
...
TEXTBOOKS_MD_DIR = './data/textbooks_md'
...
def parse_filename(filename: str) -> Tuple[str, int, int]:
    ...
    Expected format: {subject}_{grade}_chapter{number}.md
    Example: matematika_7_chapter1.md -> ('matematika', 7, 1)
    """
    try:
        ...
        raise ValueError(f"Invalid filename format: {filename}. Expected format: subject_grade_chapterN.md") from e
...
def read_markdown(md_path: str) -> str:
    ...
    try:
        ...
        raise Exception(f"Error reading markdown: {e}")
...
def chunk_text(text: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP) -> List[str]:
    ...
    For markdown, we try to preserve structure:
    - Split on double newlines (paragraphs) when possible
    - Preserve code blocks
    - Keep headings with their content
    """
    # Simple word-based chunking (same as PDF version)
    # TODO: Enhance with markdown-aware chunking
    words = text.split()
    chunks = []
...
def import_textbook_md(md_path: str, db: Client, embedding_model: SentenceTransformer,
                      entity_extractor: Optional[GrokEntityExtractor] = None):
    """Import a single markdown textbook into CozoDB with optional entity extraction."""
    ...
    print(f"   Read {len(text)} characters from markdown")
...
def main():
    ...
    print("="*60)
    print("Phase 1 POC - Markdown Textbook Import")
    print("="*60)
    ...
    # Check textbooks_md directory
    if not Path(TEXTBOOKS_MD_DIR).exists():
        print(f"\n✗ Error: Markdown directory not found: {TEXTBOOKS_MD_DIR}")
        print("Please create the directory and add markdown files")
        print("\nTo convert PDFs to markdown:")
        print("  - Use pandoc: pandoc input.pdf -o output.md")
        print("  - Use marker: https://github.com/VikParuchuri/marker")
        print("  - Use Adobe Acrobat: Export to Markdown")
        sys.exit(1)

    # Find markdown files
    md_files = list(Path(TEXTBOOKS_MD_DIR).glob('*.md'))
    if len(md_files) == 0:
        print(f"\n✗ Error: No markdown files found in {TEXTBOOKS_MD_DIR}")
        print("\nExpected filename format: subject_grade_chapterN.md")
        print("Examples:")
        print("  - matematika_7_chapter1.md")
        print("  - fisika_8_chapter2.md")
        print("  - kimia_9_chapter3.md")
        sys.exit(1)

    print(f"\nFound {len(md_files)} markdown file(s) to import")
    ...
    for md_path in md_files:
        try:
            import_textbook_md(str(md_path), db, embedding_model, entity_extractor)
            success_count += 1
        except Exception as e:
            print(f"\n✗ Error importing {md_path.name}: {e}")
            error_count += 1
            import traceback
            traceback.print_exc()
